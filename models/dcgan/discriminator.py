...
def get_dcgan_discriminator_defaults():
...
class DCGANDiscriminator(nn.Module):
    ...
    def __init__(self, n_channels: int, disc_filters: int, n_gpus: int):
        ...
        self.logger.info(f"Using DCGAN as Discriminator")
        self.ngpu = n_gpus
        self.sequential = nn.Sequential(
            # Converts input image (28, 28, n_channels) into (15, 15, n_channels)
            nn.Conv2d(
                n_channels, disc_filters, kernel_size=2, stride=2, padding=1, bias=False
            ),
