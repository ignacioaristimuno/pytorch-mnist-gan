...
class DCGANGenerator(nn.Module):
    ...
    def __init__(self, noise_dim: int, gen_filters: int, n_channels: int, n_gpus: int):
        ...
        self.sequential = nn.Sequential(
            ...
            nn.ConvTranspose2d(noise_dim, gen_filters * 8, kernel_size=4, stride=1, padding=0, bias=False),
            ...
            nn.ConvTranspose2d(gen_filters, n_channels, kernel_size=1, stride=1, padding=2, bias=False),
            nn.Tanh()
        )

    def forward(self, input):
        if input.is_cuda and self.n_gpus > 1:
            output = nn.parallel.data_parallel(self.main, input, range(self.n_gpus))
        else:
            output = self.sequential(input)
        return output


def H_out_conv_transpose_calculator(H: int, kernel: int, stride: int=1, padding: int=0, out_padding: int=0, dilatation: int=1):
    return ((H-1) * stride) - (2 * padding) + (dilatation * kernel - 1) + out_padding + 1 
